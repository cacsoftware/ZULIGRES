# -*- coding: utf-8 -*-


import sqlite3
import os
import formEAY.constantesCAC.configCAC as configCAC
from cryptography.fernet import Fernet
import formEAY.constantesCAC.configCAC as configCAC

from formEAY.utilCAC.FormatoEAY import ManipularRows


import psycopg2
import psycopg2.extras



class Conexion_base_datos():

    def func_get_datos_conexion(self):

        cadConexion = os.path.join(os.getcwd(), 'formEAY', 'dbaseCAC', 'configCAC.sqlite')  # la ruta de bd

        conn = sqlite3.connect(cadConexion)
        cur = conn.cursor()

        cad_sql = """
                            SELECT valor_item
                            FROM item_categoria
                            WHERE nom_item = 'TIPO_CONEXION'
                """

        cur.execute(cad_sql)
        row = cur.fetchone()   ## posibles valores: local, cloud
        tipo_conexion = row[0]

        if tipo_conexion == 'local':
            cad_sql = """
                            SELECT nom_item, valor_item
                            FROM item_categoria
                            WHERE categoria = 'DB LOCAL'  and activo = 1
                    """
        if tipo_conexion == 'cloud':
            cad_sql = """
                            SELECT nom_item, valor_item
                            FROM item_categoria
                            WHERE categoria = 'DB CLOUD'  and activo = 1
                                """

        cur.execute(cad_sql)
        rows = cur.fetchall()

        cur.close()
        conn.close()

        dic_conexion = ManipularRows.crearDiccionario(rows, 0, 1)
        ## ejemplo: dic_conexion = {'DBNAME': 'TEJAR_ZULIGRES', 'PUERTO': '5432', 'HOST': '127.0.0.1',
        #                           'USUARIO': 'postgres', 'PASSW': 'lebasi75'}

        return dic_conexion



    def crearConexionBD(self, TIPO_BASEDATOS ='POSTGRES'):
        cadConexion = self.func_crear_cadena_conexion()

        if TIPO_BASEDATOS=='POSTGRES':
            return psycopg2.connect(cadConexion)
        if TIPO_BASEDATOS=='SQLITE':

            #cadConexion = 'ferretariaCAC.sqlite'  # la ruta de bd
            cadConexion = os.path.join(os.getcwd(), 'formEAY', 'dbaseCAC', 'configCAC.sqlite')  # la ruta de bd

            return  sqlite3.connect(cadConexion)

    def func_crear_cadena_conexion(self):

        dic_conexion = self.func_get_datos_conexion()
        ## ejemplo: dic_conexion = {'DBNAME': 'TEJAR_ZULIGRES', 'PUERTO': '5432', 'HOST': '127.0.0.1',
        #                           'USUARIO': 'postgres', 'PASSW': 'lebasi75'}

        dbname = dic_conexion['DBNAME']
        usuario = dic_conexion['USUARIO']
        passsw = dic_conexion['PASSW']
        el_host = dic_conexion['HOST']
        puerto = dic_conexion['PUERTO']

        # dbname = 'tejar_zuligres'
        # passsw = 'lebasi75'
        # el_host = 'postgresql-8627-0.cloudclusters.net'
        # usuario = 'edinson'
        # puerto = '8627'


        cadConexion = "dbname={1} user={3} password={0}  host= {2} port={4}".format(passsw, dbname, el_host, usuario, puerto)

        print(u'Producción Zuligres,  conectado a: ', el_host)

        return cadConexion



    # def crearConexionBD_sqlite(self):
    #     #import sqlite3
    #     #cadConexion = 'configCAC.sqlite'  # la ruta de bd
    #     cadConexion = os.path.join(os.getcwd(), 'formEAY', 'dbaseCAC', 'configCAC.sqlite')  # la ruta de bd
    #     return sqlite3.connect(cadConexion)

    def crearCursorBD(self, conn, TIPO_BASEDATOS = 'POSTGRES'):
    # retorna el cursor a la db en modo diccionario
        if TIPO_BASEDATOS=='POSTGRES':
            #conn = crearConeccionBD()
            return  conn.cursor(cursor_factory=psycopg2.extras.DictCursor)

        if TIPO_BASEDATOS=='SQLITE':
            #conn = crearConeccionBD()
            conn.row_factory=sqlite3.Row
            return conn.cursor()

    def cerrarConeccionBD(self, conn):
        conn.close()

class Operaciones_SQL():

    @staticmethod
    def select_varias_filas(cad_sql, nom_funcion, cant_registros= 50, base_datos = 'POSTGRES'):
        obj_conexion_db = Conexion_base_datos()
        conn = obj_conexion_db.crearConexionBD(base_datos)
        cur = obj_conexion_db.crearCursorBD(conn, base_datos)

        try:
            cur.execute(cad_sql)
            rows = cur.fetchmany(cant_registros)

            cur.close()
            obj_conexion_db.cerrarConeccionBD(conn)


            if len(rows) > 0:
                return rows
            else:
                return None
        except psycopg2.Error as error:
            print(u' ***  No hemos podido  leer datos desde la función : {}()'.format(nom_funcion))
            print(u'    Falla --> : ', error)

    def select_una_fila(self, cad_sql, nom_funcion, base_datos = 'POSTGRES'):
        obj_conexion_db = Conexion_base_datos()
        conn = obj_conexion_db.crearConexionBD(base_datos)
        cur = obj_conexion_db.crearCursorBD(conn, base_datos)

        try:
            cur.execute(cad_sql)
            rows = cur.fetchone()
            obj_conexion_db.cerrarConeccionBD(conn)
            cur.close()

            if rows != None:
                return rows
            else:
                return None
        except psycopg2.Error as error:
            print(u' ***  No hemos podido  leer datos desde la función : {}()'.format(nom_funcion))
            print(u'    Falla --> : ', error)

class Ejecutar_SQL():

    @staticmethod
    def ejecutar_select_varios_registros(sSql, cabeceras, nom_funcion, cant_registros, base_datos = 'POSTGRES'):
        op_sql = Operaciones_SQL()
        rows = op_sql.select_varias_filas(sSql, nom_funcion, cant_registros, base_datos)
        return rows, cabeceras

    @staticmethod
    def ejecutar_select_un_registro(sSql, cabeceras, nom_funcion, base_datos = 'POSTGRES'):
        op_sql = Operaciones_SQL()
        rows = op_sql.select_una_fila(sSql, nom_funcion, base_datos)
        return rows, cabeceras

    @staticmethod
    def insert_filas(cad_sql, nom_funcion, base_datos = 'POSTGRES'):
        obj_conexion_db = Conexion_base_datos()
        conn = obj_conexion_db.crearConexionBD(base_datos)
        cur = obj_conexion_db.crearCursorBD(conn, base_datos)

        try:
            cur.execute(cad_sql)
            conn.commit()
            cur.close()
            obj_conexion_db.cerrarConeccionBD(conn)

            valor_retorno = 1

        except psycopg2.Error as error:
            print(u' ***  No hemos podido  Insertar datos desde la función : {}()'.format(nom_funcion))
            print(u'    Falla --> : ', error)
            valor_retorno = 0

        return valor_retorno

    @staticmethod
    def update_filas(cad_sql, nom_funcion, base_datos = 'POSTGRES'):
        obj_conexion_db = Conexion_base_datos()
        conn = obj_conexion_db.crearConexionBD(base_datos)
        cur = obj_conexion_db.crearCursorBD(conn, base_datos)

        try:
            cur.execute(cad_sql)
            conn.commit()
            cur.close()
            obj_conexion_db.cerrarConeccionBD(conn)

            valor_retorno = 1

        except psycopg2.Error as error:
            print(u' ***  No hemos podido  Actualizar datos desde la función : {}()'.format(nom_funcion))
            print(u'    Falla --> : ', error)
            valor_retorno = 0

        return valor_retorno


class GenerarSql():

    @staticmethod
    def crearMultiInsertSql(nom_tabla, dic_campos, list_valores):
        """
        :param nom_tabla:
        :param dic_campos: Eel estilo {´nom_empleado': 'str', 'cant':'int'}
        :param list_valores: Es una lista de listas     [[], [], []]
        :return: Un string con la Sentencia SQL completa
        """

        cad_campos = ''
        cad_valores = ''

        list_campos = dic_campos.keys()
        list_tipo_campos = list(dic_campos.values())

        for i in list_campos:
            cad_campos += str(i) + ', '
        cad_campos = cad_campos[:-2]

        for fila in list_valores:
            i = 0
            cad_valores += '('
            for dato in fila:
                tipo_dato = list_tipo_campos[i]
                if tipo_dato == 'str':
                    cad_valores += "'" + str(dato) + "', "
                if tipo_dato == 'int':
                    cad_valores += str(dato) + ", "
                i += 1
            cad_valores = cad_valores[:-2]
            cad_valores += '), '

        cad_valores = cad_valores[:-2]
        sSql = ' INSERT INTO {0} ( {1} ) VALUES {2}'.format(nom_tabla, cad_campos, cad_valores)

        return sSql

    @staticmethod
    def crearMultiUpdateSql(nom_tabla, rows, cols_busqueda, cols_a_modificar, nom_campos, tipo_campos):
        """
                EJEMPLO

                nom_tabla = 'producto'

                nom_campos = ['id', 'marca', 'ref', 'color', 'precio_detal', 'precio_mayor']

                tipo_campos = ['int', 'str', 'str', 'str', 'int', 'int']

                rows = [
                            ['1', 'AEROFLEX', '1609', 'GRIS', '500', '1000'],
                            ['2', 'AEROFLEX', '8801', 'ROJO', '600', '1200'],
                            ['3', 'AEROFLEX', 'J2268', 'NARANJADO', '400', '800'],
                            ['4', 'AEROFLEX', 'R328', 'VERDE', '550', '1100']
                        ]

                cols_busqueda = [1, 2, 3]
                cols_a_modificar = [4, 5]

                RESULTADO

                UPDATE producto SET
                        precio_detal = CASE
                                            WHEN marca = 'AEROFLEX' AND ref = '1609' AND color = 'GRIS'  THEN 500
                                            WHEN marca = 'AEROFLEX' AND ref = '8801' AND color = 'ROJO'  THEN 600
                                            WHEN marca = 'AEROFLEX' AND ref = 'J2268' AND color = 'NARANJADO'  THEN 400
                                            WHEN marca = 'AEROFLEX' AND ref = 'R328' AND color = 'VERDE'  THEN 550
                                        END,
                        precio_mayor = CASE
                                            WHEN marca = 'AEROFLEX' AND ref = '1609' AND color = 'GRIS'  THEN 1000
                                            WHEN marca = 'AEROFLEX' AND ref = '8801' AND color = 'ROJO'  THEN 1200
                                            WHEN marca = 'AEROFLEX' AND ref = 'J2268' AND color = 'NARANJADO'  THEN 800
                                            WHEN marca = 'AEROFLEX' AND ref = 'R328' AND color = 'VERDE'  THEN 1100
                                        END
                WHERE 	marca IN (AEROFLEX, AEROFLEX, AEROFLEX, AEROFLEX)  AND
                        ref IN (1609, 8801, J2268, R328)  AND
                        color IN (GRIS, ROJO, NARANJADO, VERDE)

                """

        cad = ''
        cad_WHEN = ''
        cad_CASE = ''

        for i in cols_a_modificar:
            ## craear un   CASE ... END,
            cad_WHEN = ''
            for fila in rows:
                ## crear un   WHEN  ...  THEN
                cad = ''
                for k in cols_busqueda:
                    ## enlazamos con AND cada comparacion   por ejemplo  ref = 'R155'  AND color = 'NEGRO'
                    nomCampo = nom_campos[k]
                    tipoCampo = tipo_campos[k]
                    dato = fila[k]
                    if tipoCampo == 'str':
                        dato = "'" + dato + "'"
                    cad += nomCampo + " = " + str(dato) + " AND "
                cad = cad[:-4]

                valor = fila[i]
                tipoCampo = tipo_campos[i]
                if tipoCampo == 'str':
                    valor = "'" + valor + "'"
                cad_WHEN += ' WHEN ' + cad + ' THEN ' + str(valor) + '  '

            nomCampo = nom_campos[i]
            cad_CASE += nomCampo + ' = CASE ' + cad_WHEN + ' END,'

        cad_CASE = cad_CASE[:-1]

        sSql = 'UPDATE ' + nom_tabla + ' SET ' + cad_CASE

        cad_WHERE = ''
        for j in cols_busqueda:
            nomCampo = nom_campos[j]
            cad_WHERE += nomCampo + ' IN ('
            for fila in rows:
                dato = fila[j]
                tipoCampo = tipo_campos[j]
                if tipoCampo == 'str':
                    dato = "'" + dato + "'"
                cad_WHERE += str(dato) + ', '
            cad_WHERE = cad_WHERE[:-2] + ')  AND '

        cad_WHERE = cad_WHERE[:-4]

        sSql = 'UPDATE ' + nom_tabla + ' SET ' + cad_CASE + ' WHERE ' + cad_WHERE

        return sSql


    def crearMultiUpdateSql_operaciones(nom_tabla, rows, cols_busqueda, cols_a_modificar, nom_campos, tipo_campos, dic_operaciones = {}):
        """
                EJEMPLO

                nom_tabla = 'producto'

                nom_campos = ['id', 'marca', 'ref', 'color', 'precio_detal', 'precio_mayor']

                tipo_campos = ['int', 'str', 'str', 'str', 'int', 'int']

                rows = [
                            ['1', 'AEROFLEX', '1609', 'GRIS', '500', '1000'],
                            ['2', 'AEROFLEX', '8801', 'ROJO', '600', '1200'],
                            ['3', 'AEROFLEX', 'J2268', 'NARANJADO', '400', '800'],
                            ['4', 'AEROFLEX', 'R328', 'VERDE', '550', '1100']
                        ]

                cols_busqueda = [1, 2, 3]
                cols_a_modificar = [4, 5]

                RESULTADO

                UPDATE producto SET
                        precio_detal = CASE
                                            WHEN marca = 'AEROFLEX' AND ref = '1609' AND color = 'GRIS'  THEN 500
                                            WHEN marca = 'AEROFLEX' AND ref = '8801' AND color = 'ROJO'  THEN 600
                                            WHEN marca = 'AEROFLEX' AND ref = 'J2268' AND color = 'NARANJADO'  THEN 400
                                            WHEN marca = 'AEROFLEX' AND ref = 'R328' AND color = 'VERDE'  THEN 550
                                        END,
                        precio_mayor = CASE
                                            WHEN marca = 'AEROFLEX' AND ref = '1609' AND color = 'GRIS'  THEN 1000
                                            WHEN marca = 'AEROFLEX' AND ref = '8801' AND color = 'ROJO'  THEN 1200
                                            WHEN marca = 'AEROFLEX' AND ref = 'J2268' AND color = 'NARANJADO'  THEN 800
                                            WHEN marca = 'AEROFLEX' AND ref = 'R328' AND color = 'VERDE'  THEN 1100
                                        END
                WHERE 	marca IN (AEROFLEX, AEROFLEX, AEROFLEX, AEROFLEX)  AND
                        ref IN (1609, 8801, J2268, R328)  AND
                        color IN (GRIS, ROJO, NARANJADO, VERDE)

                """

        cad = ''
        cad_WHEN = ''
        cad_CASE = ''

        for i in cols_a_modificar:
            ## craear un   CASE ... END,
            cad_WHEN = ''
            for fila in rows:
                ## crear un   WHEN  ...  THEN
                cad = ''
                for k in cols_busqueda:
                    ## enlazamos con AND cada comparacion   por ejemplo  ref = 'R155'  AND color = 'NEGRO'
                    nomCampo = nom_campos[k]
                    tipoCampo = tipo_campos[k]
                    dato = fila[k]
                    if tipoCampo == 'str':
                        dato = "'" + dato + "'"
                    cad += nomCampo + " = " + str(dato) + " AND "
                cad = cad[:-4]

                valor = fila[i]
                tipoCampo = tipo_campos[i]
                if tipoCampo == 'str':
                    valor = "'" + valor + "'"
                if dic_operaciones == {}:
                    cad_WHEN += ' WHEN ' + cad + ' THEN '  + str(valor) + '  '
                else:
                    cad_WHEN += ' WHEN ' + cad + ' THEN ' + dic_operaciones[nom_campos[i]] + ' ' + str(valor) + '  '

            nomCampo = nom_campos[i]
            cad_CASE += nomCampo + ' = CASE ' + cad_WHEN + ' END,'

        cad_CASE = cad_CASE[:-1]

        sSql = 'UPDATE ' + nom_tabla + ' SET ' + cad_CASE

        cad_WHERE = ''
        for j in cols_busqueda:
            nomCampo = nom_campos[j]
            cad_WHERE += nomCampo + ' IN ('
            for fila in rows:
                dato = fila[j]
                tipoCampo = tipo_campos[j]
                if tipoCampo == 'str':
                    dato = "'" + dato + "'"
                cad_WHERE += str(dato) + ', '
            cad_WHERE = cad_WHERE[:-2] + ')  AND '

        cad_WHERE = cad_WHERE[:-4]

        sSql = 'UPDATE ' + nom_tabla + ' SET ' + cad_CASE + ' WHERE ' + cad_WHERE

        return sSql
